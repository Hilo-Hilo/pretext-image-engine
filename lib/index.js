...
var PretextImageEngine = class {
  ...
  applyMasked(result, statusText = "Masked layout active") {
    ...
    if (this.scene.reveal.unit !== "line") {
      this.lineElements.forEach((el) => {
        this.splitTextIntoUnits(el, this.scene.reveal.unit);
      });
    }
    this.applyProgressProjection();
    this.emitRender();
  ...
  splitTextIntoUnits(el, unit) {
    const text = el.textContent ?? "";
    if (!text) return;
    el.textContent = "";
    const className = unit === "char" ? "pie-char" : "pie-word";
    const tokens = unit === "char" ? [...text] : text.match(/\s+|\S+/g) ?? [];
  ...
  applyFallback(label) {
    ...
    if (this.scene.reveal.unit !== "line") {
      this.fallbackContent.querySelectorAll(".pie-fallback-block").forEach(
        (el) => this.splitTextIntoUnits(el, this.scene.reveal.unit)
      );
      this.applyProgressProjection();
    }
    this.emitRender();
